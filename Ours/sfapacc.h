...
public: 
    ...
    sfapacc(uint32_t _MEMORY, int _HASH_NUM, int _windowsize, int __alpha = 1, string _name = "SPLIT_O") {
        NAME = _name;
        MEMORY = _MEMORY;
        HASH_NUM = _HASH_NUM;
    ...
    ~sfapacc() {
      ...
      float setpara(int i, int j) {
           if(cc[i][j].count > small/(4*cell_num)) {
                   return 10;
		// return 1000;
           } else {
                  return 1;
    ...
    void Insert(DATA_TYPE item) {  
		...
		bitmap->Set(minHash*LENGTH +minPos);
	     ...
	     if(rng() % ccss == 0) {
	      psum += 1;
	      small = small + 1 - cc[minHash][minPos].count;
	      cc[minHash][minPos].item = item;
	      cc[minHash][minPos].count = 1;
	      }
