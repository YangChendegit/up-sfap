#ifndef STREAMSUMMARY2_H
#define STREAMSUMMARY2_H
...
class StreamSummary2{
public:
    typedef std::unordered_map<DATA_TYPE, COUNT_TYPE> HashMap;
    typedef std::unordered_map<DATA_TYPE, TIME_TYPE> HashMap_T;
    class DataNode;
    class CountNode;
    ...
    static uint32_t Memory2Size(uint32_t memory){
        return memory / ((sizeof(DATA_TYPE) + sizeof(DataNode*)) / LOAD
                         + sizeof(DATA_TYPE) + sizeof(COUNT_TYPE) + 4 * sizeof(void*) + sizeof(TIME_TYPE) + sizeof(DATA_TYPE));
    }
    ...
    void SS_Replace(const DATA_TYPE& data, DATA_TYPE &min_ori){
