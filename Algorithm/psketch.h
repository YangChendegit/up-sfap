...
public: 
    ...
    psketch(uint32_t _MEMORY, int _HASH_NUM, int _windowsize, string _name = "SPLIT_O") {
        NAME = _name;
        MEMORY = _MEMORY;
        HASH_NUM = _HASH_NUM;
    ...
    void Insert(DATA_TYPE item) {
	    ...
	    else if(rng() %(18*(cc[minHash][minPos].count + cc[minHash][minPos].hotcount) +1) == 0) {
                cc[minHash][minPos].item = item;
                cc[minHash][minPos].count += 1;
                cc[minHash][minPos].hotcount += 1;
		cc[minHash][minPos].status = 1;
    ...
    void NewWindow() {
	   for(int i = 0; i < HASH_NUM; i++) { 
		   for(int j = 0; j < LENGTH; j++) {
	               if(cc[i][j].status ==0 ) {
			   cc[i][j].hotcount = (cc[i][j].hotcount > 1 ? cc[i][j].hotcount-1:0);
			 }
			   cc[i][j].status = 0;
	           }
	   }
