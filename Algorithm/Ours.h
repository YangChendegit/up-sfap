...
public:
    ...
    Ours(uint32_t _MEMORY, TIME_TYPE _low = 1, TIME_TYPE _high = 100,  double ratio = 0.15, std::string _name = "Ours"){
        NAME = _name;
        MEMORY = _MEMORY;
    ...
    repMap Rep(COUNT_TYPE HIT)  {
	 ...
	 for(uint32_t i = 0; i < HEAVY_LENGTH; ++i) {
	     for(uint32_t j = 0; j < CELLNUM; ++j) {
		  if(temp.find(buckets[i].cells[j].item) != temp.end()) {
			  temp[buckets[i].cells[j].item] += buckets[i].cells[j].count;
		  } else {
			  temp[buckets[i].cells[j].item] = buckets[i].cells[j].count;
		  }
	     }
