...
public: 
    ...
    //XXX
    pontus(uint32_t _MEMORY, int _HASH_NUM, int _windowsize, string _name = "SPLIT_O") {
        NAME = _name;
        MEMORY = _MEMORY;
        HASH_NUM = _HASH_NUM;
        cell_num = _MEMORY / (sizeof(int) + sizeof(DATA_TYPE) + 2*sizeof(uint8_t));
    ...
    void Insert(DATA_TYPE item) {
	    ...
	    else if(rng() %(cc[minHash][minPos].count  +1) == 0 && cc[minHash][minPos].hotcount == 0) {
		...
		cc[minHash][minPos].hotcount = 1;
			...
			cc[minHash][minPos].count = 1;
                        //cc[minHash][minPos].hotcount = (cc[minHash][minPos].hotcount > 1 ? cc[minHash][minPos].hotcount-1:0);
			cc[minHash][minPos].status = 1;
                }
